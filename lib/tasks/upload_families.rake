...
task :upload_families => :environment do
  ...
  ['ww_week1.csv', 'ww_week2.csv', 'ww_waitlist.csv'].each do |filename|
    ...
    csv.each do |row|
      ...
      print week
      family = Family.create!(lottery:row[0], name: row[9], phone: row[10], email: row[11], adults: row[6], kids: row[7], week: week, week1: week1, week2: week2, week3: week3)
